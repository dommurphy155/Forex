...
async def daily(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ops = await get_open()
    if not ops:
        await update.message.reply_text("📭 No open trades for today, no P&L to report.", parse_mode=None)
        return
    bal = await get_balance()
...
async def open_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ops = await get_open()
    if not ops:
        await update.message.reply_text("📭 No open trades.", parse_mode=None)
        return
    bal = await get_balance()
    msgs = []
    for t in ops:
    ...
    await update.message.reply_text("\n".join(msgs), parse_mode=None)

async def maketrade(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global last
    last = datetime.utcnow()
    await update.message.reply_text("⚙️ Trade request acknowledged.\nAuto-trading operates on a 5-min cycle.", parse_mode=None)
    try:
        await tick()
        await update.message.reply_text("✅ Trade tick executed.", parse_mode=None)
    except Exception as e:
        logger.error(f"Error during trade tick: {e}")
        await update.message.reply_text(f"❌ Error executing trade tick: {e}", parse_mode=None)

async def dummytrade(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        await place_dummy_trade()
        await update.message.reply_text("✅ Dummy trade placed successfully.", parse_mode=None)
    except Exception as e:
        logger.error(f"Error placing dummy trade: {e}")
        await update.message.reply_text(f"❌ Failed to place dummy trade: {e}", parse_mode=None)

async def closeall(update: Update, context: ContextTypes.DEFAULT_TYPE):
    closed_trades = await close_all_trades()
    if not closed_trades:
        await update.message.reply_text("🛑 No trades to close.", parse_mode=None)
        return
